...
**快速入门推荐书籍**：https://changkun.de/modern-cpp/pdf/modern-cpp-tutorial-zh-cn.pdf
...
vector<int> getTwoSum(vector<int>& nums, int target) {
    ...
    unordered_map<int,int> umap; // data index
    for(int i = 0; i < nums.size(); i++>) {
        if(umap[target-nums[i]] != umap.end()) {
...
- std::smatch 的本质其实是std::match_results，在标准库中， std::smatch 被定义为了 std::match_results，也就是一个子串迭代器类型的 match_results。使用 std::smatch 可以方便的对匹配的结果进行获取，例如：
...
使用小括号来指定子表达式（也叫作分组）。
**在正则表达式中 （）表示把内容拿出来**。可以用于细分匹配结果，取出匹配结果中关键的几项。
...
```c
int main(void) {
	std::string str = "Hello2019-05-06, 3021-05-07, 4023-04-08";
	std::smatch match; //搜索的结果
	std::regex pattern("(\\d{4})-(\\d{1,2})-(\\d{1,2})");
	
	//只需要利用迭代器就可以很轻松的访问到所有匹配的结果值。
	std::string::const_iterator citer = str.cbegin();
	while (std::regex_search(citer, str.cend(), match, pattern)) {
...
```
std::regex_search返回 std::smatch, 其中std::smatch中每个元素都是一个pair<std::string::const_iterator, std::string::const_iterator>，用于存储匹配字串开始的位置startPos，及其结束位置的下一位endPos，范围为：[startPos, endPos)

**特别注意：std::string::const_iterator没有重载输出操作符，输出时加***

## **std::regex_replace**

replace是替换匹配，即可以将符合匹配规则的子字符串替换为其他字符串。
```cpp
std::string str = "Hello_2023";
std::regex pattern("Hello");

// regex_replace(src_str, regex_pattern, replace_str)
std::cout << std::regex_replace(str, pattern, "") << std::endl;
//_2023
```

### 扩展用法

除了直接替换以外，还可以用来调整字符串的内容（缩短、字符顺序等）

注：使用（）子表达式后，不同项就是从编号1开始

```cpp
std::string str = "Hello_2023";
std::regex pattern("(.{3})(.{2})_(\\d{4})");
//缩短
std::cout << std::regex_replace(str, pattern, "$1$3") << std::endl; //Hel2023
//更换顺序
std::cout << std::regex_replace(str, pattern, "$1$3$2") << std::endl; //Hel2023lo
```
...
std::regex pattern("a*", std::regex::icase);
std::regex pattern2("a*");
std::cout << std::regex_match(str, pattern) << std::endl;
std::cout << std::regex_match(str, pattern2) << std::endl;
```

### regex帮助网站：
- https://tool.oschina.net/regex/
- 显示正则表达式匹配结构：https://regexper.com/ 
...
override关键字的作用是：在成员函数声明或者定义中，override确保该函数为虚函数并覆盖来自基类的虚函数。

## 使用后的好处
- 可以当注释用，方便阅读
- 告诉阅读代码的人，这是方法的重写
- 编译器可以给你验证 override 对应的方法名是否是父类所有的，如果没有则报错。

在重写方法时，最好加上override关键字，以强化代码规范。
...
以上四类函数控制着类实例的创建、初始化、拷贝以及销毁。

我们知道当创建了有参的构造函数后，如果想调用无参构造函数编译是会报错的。因为一旦定义了构造函数，系统的默认构造函数就会被屏蔽掉。c++11中通过定义了default关键字让默认构造函数恢复。
